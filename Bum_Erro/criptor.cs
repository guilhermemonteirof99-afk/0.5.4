using System;
using System.Security.Cryptography;


namespace bum_erro;


public class criptor
{
   private readonly byte[] Sbox = new byte[256]
   {
       
   };
  
   // A Tabela Oficial de Substituição Reversa do AES (Inverse S-Box)
   private readonly byte[] InvSbox = new byte[256]
   {
       
   };
  
  
   // 1. DIFUSÃO CAÓTICA: Mapa do Gato de Arnold (Agora dependente da rodada)
  
   private byte[] ArnoldCatMap(byte[] bloco, int r)
   {
       byte[] resultado = new byte[16];
       for (int y = 0; y < 4; y++)
       {
           for (int x = 0; x < 4; x++)
           {
               // O + r destrói o looping de período 3
               int novoX = (x + y + r) % 4;
               int novoY = (x + 2 * y + r) % 4;
               resultado[novoY * 4 + novoX] = bloco[y * 4 + x];
           }
       }
       return resultado;
   }




   private byte[] InvArnoldCatMap(byte[] bloco, int r)
   {
       byte[] resultado = new byte[16];
       for (int y = 0; y < 4; y++)
       {
           for (int x = 0; x < 4; x++)
           {
               // Calcula as coordenadas originais de onde o byte veio
               int oldX = ((2 * x - y - r) % 4 + 4) % 4;
               int oldY = ((-x + y) % 4 + 4) % 4;
          
               // Pega o byte da posição atual (x, y) no bloco cifrado
               // e DEVOLVE para a posição original (oldX, oldY) no resultado.
               resultado[oldY * 4 + oldX] = bloco[y * 4 + x];
           }
       }
       return resultado;
   }
   // 2. DIFUSÃO DE VALORES: Cascata XOR (Efeito Avalanche Intra-Bloco)
  
   private byte[] MisturarValores(byte[] bloco)
   {
       byte[] resultado = new byte[16];
       resultado[0] = bloco[0];
      
       for (int i = 1; i < 16; i++)
       {
           resultado[i] = (byte)(bloco[i] ^ resultado[i - 1]);
       }
      
       // Elo final para fechar o ciclo
       resultado[0] = (byte)(resultado[0] ^ resultado[15]);
       return resultado;
   }


   private byte[] DesfazerMistura(byte[] bloco)
   {
       byte[] resultado = new byte[16];
      
       byte primeiroOriginal = (byte)(bloco[0] ^ bloco[15]);
       resultado[0] = primeiroOriginal;
      
       for (int i = 15; i > 0; i--)
       {
           resultado[i] = (byte)(bloco[i] ^ bloco[i - 1]);
       }
      
       resultado[1] = (byte)(bloco[1] ^ primeiroOriginal);
       return resultado;
   }


  
   // GERENCIAMENTO DE CHAVES, PADDING E HASH
  
   public byte[] chave { private set; get; }
   public byte[] IV { get; private set; }
  
   public criptor()
   {
       IV = new byte[16];
       chave = new byte[16];
   }
  
   public string GerarChaveSegura()
   {
       RandomNumberGenerator.Fill(chave);
       return Convert.ToBase64String(chave);
   }


   public byte[] AplicarPadding(byte[] arquivoOriginal)
   {
       int sobra = arquivoOriginal.Length % 16;
       byte bytesFaltando = (byte)(16 - sobra);


       int novoTamanho = arquivoOriginal.Length + bytesFaltando;
       byte[] arquivoComPadding = new byte[novoTamanho];


       Array.Copy(arquivoOriginal, arquivoComPadding, arquivoOriginal.Length);


       for (int i = arquivoOriginal.Length; i < arquivoComPadding.Length; i++)
       {
           arquivoComPadding[i] = bytesFaltando;
       }


       return arquivoComPadding;
   }


   public byte[] RemoverPadding(byte[] arquivoComPadding)
   {
       int bytesInjetados = arquivoComPadding[arquivoComPadding.Length - 1];


       // TRAVA DE SEGURANÇA: Impede o Overflow e avisa que a senha/arquivo estão errados.
       if (bytesInjetados < 1 || bytesInjetados > 16 || bytesInjetados > arquivoComPadding.Length)
       {
           throw new CryptographicException("Falha na Descriptografia! A chave está errada, o IV não bate, ou o arquivo '.trancado' foi corrompido.");
       }


       int tamanhoOriginal = arquivoComPadding.Length - bytesInjetados;
       byte[] arquivoLimpo = new byte[tamanhoOriginal];
  
       Array.Copy(arquivoComPadding, arquivoLimpo, tamanhoOriginal);
       return arquivoLimpo;
   }


   public string lacreHash(byte[] arquivoOriginal)
   {
       using (SHA256 sha256 = SHA256.Create())
       {
           byte[] hash = sha256.ComputeHash(arquivoOriginal);
           return Convert.ToBase64String(hash);
       }
   }
  
   public string vetorInicial()
   {
       RandomNumberGenerator.Fill(IV);
       return Convert.ToBase64String(IV);
   }


   public void carregarChave(string chavebase, string ivbase)
   {
       byte[] chaveRecuperada = Convert.FromBase64String(chavebase);
       byte[] ivRecuperado = Convert.FromBase64String(ivbase);
      
       Array.Copy(chaveRecuperada, 0, chave, 0, 16);
       Array.Copy(ivRecuperado, 0, IV, 0, 16);
   }
  
   // KEY SCHEDULE
   private byte[][] GerarSubChaves()
   {
       byte[][] subchaves = new byte[10][];
       for (int r = 0; r < 10; r++)
       {
           subchaves[r] = new byte[16];
           for (int b = 0; b < 16; b++)
           {
               subchaves[r][b] = (byte)(chave[(b + r) % 16] ^ (r + 1));
           }
       }
       return subchaves;
   }
  
  
   // MOTORES DE CRIPTOGRAFIA
  
   public byte[] Criptografia_Pessoal_Modo_CBC(byte[] arquivoOriginal)
   {
       byte[] arquivoPronto = AplicarPadding(arquivoOriginal);
       byte[] arquivoCifrado = new byte[arquivoPronto.Length];
      
       byte[] vetorCorrente = new byte[16];
       Array.Copy(IV, vetorCorrente, 16);


       byte[][] subchaves = GerarSubChaves();
       int rodadas = 10;


       for (int i = 0; i < arquivoPronto.Length; i += 16)
       {
           byte[] blocoAtual = new byte[16];
           Array.Copy(arquivoPronto, i, blocoAtual, 0, 16);


          
           for (int b = 0; b < 16; b++)
           {
               blocoAtual[b] = (byte)(blocoAtual[b] ^ vetorCorrente[b]);
           }
           
           for (int r = 0; r < rodadas; r++)
           {
              
               for (int b = 0; b < 16; b++)
               {
                   blocoAtual[b] = (byte)(blocoAtual[b] ^ subchaves[r][b]);
               }
               
               for (int b = 0; b < 16; b++)
               {
                   blocoAtual[b] = Sbox[blocoAtual[b]];
               }
              
               blocoAtual = MisturarValores(blocoAtual);
              
               blocoAtual = ArnoldCatMap(blocoAtual, r);
           }


           Array.Copy(blocoAtual, 0, arquivoCifrado, i, 16);
           Array.Copy(blocoAtual, 0, vetorCorrente, 0, 16);
       }
       return arquivoCifrado;
   }


   public byte[] DesCriptografia_Pessoal_Modo_CBC(byte[] arquivocifra)
   {
       byte[] arquivoProntoDestrancado = new byte[arquivocifra.Length];
      
       byte[] PuxaVI = new byte[16];
       Array.Copy(IV, PuxaVI, 16);


       byte[][] subchaves = GerarSubChaves();
       int rodadas = 10;
      
       for (int i = 0; i < arquivocifra.Length; i += 16)
       {
           byte[] blocoAtual = new byte[16];
           Array.Copy(arquivocifra, i, blocoAtual, 0, 16);
          
           byte[] proximovetor = new byte[16];
           Array.Copy(blocoAtual, proximovetor, 16);
          
          
           for (int r = rodadas - 1; r >= 0; r--)
           {
              
               blocoAtual = InvArnoldCatMap(blocoAtual, r);
               
               blocoAtual = DesfazerMistura(blocoAtual);
              
              
               for (int b = 0; b < 16; b++)
               {
                   blocoAtual[b] = InvSbox[blocoAtual[b]];
               }


              
               for (int b = 0; b < 16; b++)
               {
                   blocoAtual[b] = (byte)(blocoAtual[b] ^ subchaves[r][b]);
               }
           }


          
           for (int b = 0; b < 16; b++)
           {
               blocoAtual[b] = (byte)(blocoAtual[b] ^ PuxaVI[b]);
           }
          
           Array.Copy(blocoAtual, 0, arquivoProntoDestrancado, i, 16);
           Array.Copy(proximovetor, 0, PuxaVI, 0, 16);
       }
      
       return RemoverPadding(arquivoProntoDestrancado);
   }
}
